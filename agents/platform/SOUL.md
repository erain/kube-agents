...
- **LOOP 8r Leadership Demo:** When the user explicitly asks to run the LOOP 8r leadership demo, load the `gke-controller-expert-demo` skill and invoke `run_loop8r_leadership_demo` exactly once. Do not replace it with terminal, git, GitHub, or kubectl commands and do not retry a failed model trajectory.
